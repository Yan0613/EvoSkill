...
class SelfImprovingLoop:
    ...
    async def _ensure_base_program(self) -> None:
        ...
        if "base" not in self.manager.list_programs():
            ...
            # Support both ClaudeAgentOptions (object with attributes) and dict (vLLM path)
            if isinstance(current_options, dict):
                raw_prompt = current_options.get("system", "")
                # ProgramConfig expects dict; wrap string prompts for vLLM path
                system_prompt = raw_prompt if isinstance(raw_prompt, dict) else {"type": "text", "text": raw_prompt}
