#!/usr/bin/env python3
"""
Trace a single SEAL-QA sample with vLLM backend.
Records full input, output, tool calls, and timing to a JSON file.

Usage:
    python3 scripts/trace_sealqa_single.py \
        --vllm-base-url http://29.206.1.136:8765/v1 \
        --sample-index 0 \
        --output results/trace_sealqa_single.json
"""
import argparse
import asyncio
import json
...
async def main():
    parser = argparse.ArgumentParser(description="Trace a single SEAL-QA sample with vLLM")
    parser.add_argument("--dataset", type=Path, default=Path(".dataset/seal-0.csv"))
    parser.add_argument("--sample-index", type=int, default=0, help="Row index in CSV to evaluate (default: 0)")
    parser.add_argument("--topic", type=str, default=None, help="Filter by topic before selecting index")
    ...
    parser.add_argument("--output", type=Path, default=Path("results/trace_sealqa_single.json"))
    ...
    from src.agent_profiles import set_sdk, set_vllm_config, make_sealqa_agent_options, Agent
    ...
    if args.topic:
        data = data[data["topic"] == args.topic].reset_index(drop=True)
        print(f"[Filter] topic={args.topic}, {len(data)} samples remaining")
    ...
    print(f"\n[Sample] index={args.sample_index}, topic={topic}, freshness={freshness}")
    ...
    score = score_sealqa(question, answer, final_answer)
    correct = score > 0
    print(f"\n[Result]")
