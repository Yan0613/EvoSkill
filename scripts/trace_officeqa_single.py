#!/usr/bin/env python3
"""
Trace a single OfficeQA sample with vLLM backend.
Records full input, output, tool calls, and timing to a JSON file.

Usage:
    python3 scripts/trace_officeqa_single.py \
        --vllm-base-url http://29.206.1.136:8765/v1 \
        --sample-index 0 \
        --output results/trace_officeqa_single.json
"""
import argparse
import asyncio
import json
...
async def main():
    parser = argparse.ArgumentParser(description="Trace a single OfficeQA sample with vLLM")
    parser.add_argument("--dataset", type=Path, default=Path(".dataset/officeqa/officeqa_pro.csv"))
    ...
    parser.add_argument("--output", type=Path, default=Path("results/trace_officeqa_single.json"))
    ...
    uid = row.get("uid", f"sample_{args.sample_index}")
    ...
    print(f"\n[Sample] index={args.sample_index}, uid={uid}, difficulty={difficulty}")
    print(f"[Question] {question}")
    print(f"[Ground Truth] {answer}")
    if source_docs:
        print(f"[Source Docs] {source_docs}")

    # Create agent (reuse sealqa agent which supports vllm)
